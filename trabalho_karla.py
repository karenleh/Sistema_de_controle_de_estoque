...
print("A cidade de %s tem %d habitantes." % (cidade, populacao))

produto = "Livro"
preco = 49.99
print("0 {} custa R$(:.2f).".format (produto, preco))

def obter_dados_item():
    item = input("\nDigite o nome do item: ").strip()
    unidade = input("Unidade (ex: kg, un): ").strip()

    while True:
        try:
            valor = float(input("Valor unitário (use ponto para decimais): "))
            break  # Sai do loop se a conversão para float der certo
        except ValueError:
            print("Erro: Por favor, digite um número válido para o valor.")

    return {"item": item, "Unidade": unidade, "Valor unitário": valor}

def adicionar_ao_estoque(estoque_lista, produto):
    estoque_lista.append(produto)
    print(f"\n✅ {produto['item']} adicionado ao estoque com sucesso!")
...
def exibir_tabela_estoque(estoque_lista):
    ...
    print("\n" + "="*50)
    print(f"{'ITEM':<20} | {'UNID.':<10} | {'VALOR UNIT.':>12}")
    print("-" * 50)

    for p in estoque_lista:
        # Formata o valor como R$ 0.00
        valor_formatado = f"R$ {p['Valor unitário']:>10.2f}"
        print(f"{p['item']:<20} | {p['Unidade']:<10} | {valor_formatado}")
    print("="*50)

# Fluxo Principal
estoque = []

print("--- Sistema de Cadastro de Estoque ---")
